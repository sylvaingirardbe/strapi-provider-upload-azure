...
module.exports = {
  ...
  auth: {
    ...
    azureType: {
      label: 'Azure Type',
      type: 'enum',
      values: Object.values(azureTypes).map(x => x.label)
    }
  },
  init: (config) => {
    const blobService = azurestorage.createBlobService(config.connectionString);
    const azureTypeUrl = Object.values(azureTypes).find(x => x.label === config.azureType);
      ...
      upload: (file) => {
        return new Promise((resolve, reject) => {
          ...
          fileStream.pipe(writeStream).on('finish', () => {
            file.url = `https://${config.account}.blob.core.${azureTypeUrl ? azureTypeUrl.url : azureTypes.base.url}/${config.container}/${filename}`;
